...
def npcp_square_matmul_bench():
    ...
    for i in range(5):
        for dim in dims:
            ...
            x = np.random.rand(batch_size, dim, dim).astype(np.float32)
            y = np.random.rand(batch_size, dim, dim).astype(np.float32)
    ...
    f = open('numpy_cupy_matmul_square_cpu_vs_gpu_float32.csv', 'w')
    writer = csv.writer(f)
