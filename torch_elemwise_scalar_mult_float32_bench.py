...
def torch_elemwise_mult_bench():
  ...
  for i in range(5):
    for dim in dims:
      ...
      z_gpu = torch.mul(x_gpu, 99)
      timer.end(t)

      assert (z_gpu[0][0][0] == x_gpu[0][0][0] * 99)
  ...
  f = open('torch_elemwise_scalar_mult_cpu_vs_gpu_float32.csv', 'w')
  writer = csv.writer(f)
  writer.writerows(data)
  f.close()

if __name__=='__main__':
  torch_elemwise_mult_bench()