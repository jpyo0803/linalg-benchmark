...
def npcp_elemwise_add_bench():
    ...
    for i in range(5):
        for dim in dims:
            ...
            x = np.random.randint(0, 127, (batch_size,
                                  dim, dim), dtype=np.uint32)
            y = np.random.randint(0, 127, (batch_size,
                                  dim, dim), dtype=np.uint32)
    ...
    f = open('numpy_cupy_elemwise_add_cpu_vs_gpu_uint32.csv', 'w')
    writer = csv.writer(f)
