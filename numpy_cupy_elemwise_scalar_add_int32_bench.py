...
def npcp_elemwise_scalar_add_bench():
    ...
    for i in range(5):
        for dim in dims:
            ...
            z_gpu = cp.add(x_gpu, 7)
            timer.end(t)
            assert (z_gpu[0][0][0] == x_gpu[0][0][0] + 7)
    ...
    f = open('numpy_cupy_elemwise_scalar_add_cpu_vs_gpu_int32.csv', 'w')
    writer = csv.writer(f)
...
if __name__ == '__main__':
    npcp_elemwise_scalar_add_bench()
