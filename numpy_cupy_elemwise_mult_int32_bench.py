...
def npcp_elemwise_mult_bench():
    ...
    for i in range(5):
        for dim in dims:
            ...
            y = np.random.randint(-128, 127, (batch_size,
                                  dim, dim), dtype=np.int32)
    ...
    f = open('numpy_cupy_elemwise_mult_cpu_vs_gpu_int32.csv', 'w')
    writer = csv.writer(f)
