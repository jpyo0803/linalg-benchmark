...
def npcp_elemwise_scalar_mult_bench():
    ...
    for i in range(5):
        for dim in dims:
            ...
            x = np.random.randint(-16, 16, (batch_size,
            ...
            z_gpu = cp.multiply(x_gpu, 4)
            timer.end(t)
            assert (z_gpu[0][0][0] == x_gpu[0][0][0] * 4)
    ...
    f = open('numpy_cupy_elemwise_scalar_mult_cpu_vs_gpu_int8.csv', 'w')
    writer = csv.writer(f)
