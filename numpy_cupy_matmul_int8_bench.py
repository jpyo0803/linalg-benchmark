...
def npcp_square_matmul_bench():
  ...
  for i in range(5):
    for dim in dims:
      ...
      x = np.random.randint(-128, 127, (batch_size, dim, dim), dtype=np.int8)
      y = np.random.randint(-128, 127, (batch_size, dim, dim), dtype=np.int8)
  ...
  f = open('numpy_cupy_matmul_square_cpu_vs_gpu_int8.csv', 'w')
  writer = csv.writer(f)
