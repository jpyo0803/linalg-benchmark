...
def torch_square_matmul_bench():
  ...
  for i in range(5):
    for dim in dims:
      ...
      x = torch.randn((batch_size, dim, dim), dtype=torch.float64)
      y = torch.randn((batch_size, dim, dim), dtype=torch.float64)
  ...
  f = open('torch_matmul_square_cpu_vs_gpu_float64.csv', 'w')
  writer = csv.writer(f)
